...
//        this.authme = config.getBoolean("auth-me");
        ...
        SCHEDULED = Bukkit.getScheduler().runTaskTimerAsynchronously(BukkitBootstrap.getInstance(), () -> {
            if (joins > 0) -- joins;
...
//        if (authme && AuthMeApi.getInstance().isRegistered(event.getName())) return;
        ...
        authList.add(Bukkit.getScheduler().runTaskLater(BukkitBootstrap.getInstance(), () ->
