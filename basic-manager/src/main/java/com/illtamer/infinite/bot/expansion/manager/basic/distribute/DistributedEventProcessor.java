...
public class DistributedEventProcessor<T> {
     ...
     * 创建广播监听器
     */
    public class DistributedEventListener implements com.illtamer.infinite.bot.minecraft.api.event.Listener {
        ...
        public void onClientBroadcast(ClientBroadcastEvent event) {
            String dataKey = buildDataKey(event);
            String tag = extractTag(dataKey);
            String eventKey = extractEventKey(tag);
            ...
            // 执行事件处理并返回结果
            DistributedEventContext context = parseContext(event.getData());
            T result = handler.apply(context);
        ...
        public void onClientBroadcastCallback(ClientBroadcastCallbackEvent event) {
            String cacheKey = buildCacheKey(event.getClient(), extractEventKeyFromCallback(event.getData()));
            blockingCache.put(cacheKey, event.getData());
        }
    }
    ...
    private Pair<List<T>, List<Client>> collectClientData(String eventKey, DistributedEventContext context,
        ...
        // 发送广播到所有其他客户端
        String broadcastData = buildBroadcastData(eventKey, context);
        for (Client targetClient : targetClientList) {
            String dataKey = buildDataKey(client, targetClient, expansionName, eventKey);
            OpenAPIHandling.sendBroadcastData(dataKey + "@@" + broadcastData, targetClient);
        }

        // 收集回调数据
        for (Client targetClient : targetClientList) {
            try {
                String cacheKey = buildCacheKey(targetClient, expansionName, eventKey);
    ...
    private String buildBroadcastData(String eventKey, DistributedEventContext context) {
        return gson.toJson(new BroadcastPayload(eventKey, context));
    }

    private String buildDataKey(Client source, Client target, String expansionName, String eventKey) {
        return String.join("#", expansionName, eventKey, source.getAppId(), target.getAppId());
    }

    private String buildCacheKey(Client client, String expansionName, String eventKey) {
        return String.join("#", client.getAppId(), expansionName, eventKey);
    }

    private String extractEventKey(String tag) {
        String[] parts = tag.split("#");
        return parts.length >= 2 ? parts[1] : "";
    }

    private String extractTag(String dataKey) {
        String[] parts = dataKey.split("#");
        return parts.length >= 2 ? parts[0] + "#" + parts[1] : "";
    }

    private boolean isValidBroadcast(String dataKey, Client source, Client target) {
        String expectedKey = buildDataKey(source, target, expansionName, "");
        return dataKey.startsWith(expectedKey);
    }

    private DistributedEventContext parseContext(String data) {
        return gson.fromJson(data, DistributedEventContext.class);
    }

    private String buildDataKey(ClientBroadcastEvent event) {
        return event.getData().split("@@")[0];
    }
    ...
    private String extractEventKeyFromCallback(String data) {
        // 从回调数据中提取事件key
        return "";
    }
    ...
    private static class BroadcastPayload {
        ...
        public BroadcastPayload(String eventKey, DistributedEventContext context) {
